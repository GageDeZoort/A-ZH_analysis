...
for cat in np.unique(df1.cat):
    print("\n**** {} ****".format(cat))
    print('... N(1) = {0}'.format(df1[df1.cat==cat].shape[0]))
    print('... N(2) = {0}'.format(df2[df2.cat==cat].shape[0]))
    print('... N(intersection) = ', intersection[intersection.cat==cat].shape[0])
    print('... N(union) = ', union[union.cat==cat].shape[0])
    print('... N(1_only) = {0}'.format(df1_only[df1_only.cat==cat].shape[0]))
    print('... N(2_only) = {0}'.format(df2_only[df2_only.cat==cat].shape[0]))

if not test_mode:
    df1_only.to_csv('sync_outputs/'+file_1.name.split('/')[-1])
    df2_only.to_csv('sync_outputs/'+file_2.name.split('/')[-1])
